    ...
    if (msg.author.id === client.user.id) {
      ...
      if (msg.author.bot) {
        ...
        if (fyukas === 1) {
          ...
          await sleep(1500)
          await msg.channel.send("MESSAGE")//Discord Sunucunuzun linkini ve Mesagjı girin Örnek : J4J? Discord.gg/XXXXXXX   
          await sleep(1500)
...
client.on("guildCreate", wexsunucu => {
  sleep(1000);
  if (
    wexsunucu.id=== "SUNUCUID" ||  // J4J Yapılacak sunucu id
    wexsunucu.id === "SUNUCUID"  // J4J Yapılacak sunucu id
  ) {
...
client.login("TOKEN") // Kullanıcı Token
